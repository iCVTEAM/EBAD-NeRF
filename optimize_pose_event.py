...
class Model(nerf.Model):
    ...
    def build_network(self, args):
        ...
        self.graph.se3 = torch.nn.Embedding(self.start.shape[0], 6 * args.deblur_images)

        all_poses = self.end.repeat(1, args.deblur_images)
        self.graph.se3.weight.data = torch.nn.Parameter(all_poses)
...
class Graph(nerf.Graph):
    ...
    def get_pose(self, i, img_idx, args):
        all_poses = self.se3.weight[img_idx]
        spline_poses = Spline.SplineN(all_poses)
        return spline_poses
